...
```
Note: While the "clean" extension renders to HTML format, this demo includes an example where a LaTeX table can be embedded as an image, which requires [TinyTex](https://quarto.org/docs/output-formats/pdf-engine.html).
...
::::{.fragment}
P.S. Quarto provides dedicated
[math environments](https://quarto.org/docs/authoring/cross-references.html#theorems-and-proofs)
for theorems, lemmas, etc. But for presentations, it's easier just to highlight via a
...
Regression table software should play nice with this theme out the
gate. Here's a [modelsummary](https://modelsummary.com/)
example.
...
(The table is displayed on the next slide.)

## Regression tables

### fixest::etable

**Aside:** We estimated our models with [fixest](https://lrberge.github.io/fixest/), which bundles its own powerful `etable` function. While `etable` was designed for LaTeX output, it can be [incorporated](https://lrberge.github.io/fixest/articles/etable_new_features.html) into HTML Quarto too.^[You will need to install the `tinytex` & `pdftools` packages first.]

- Set the `output: asis` R chunk option in your Quarto doc.
- Set the `setFixest_etable(markdown = TRUE)` option.
